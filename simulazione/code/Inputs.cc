...
void AN_Input::hadrpars(double & Kref, double & Mref, double & KerrRef, double & MerrRef, double & dKu, double & dMu) const {
  ...
  if (iparam == 0) {
    ...
    Kref = 0.00720;
    Mref = 0.0525;      // GeV^2
    KerrRef = 0.00037;  // matched to the expected 1sigma in the ideal case
    MerrRef = 0.0030;   //  "   "
    if (iLumi==1) {
      KerrRef = 0.0025; // matched to the expected 1sigma in the ideal case for the PilotRun lumi = 3/pb
      MerrRef = 0.;     // 1D param 
    }
  }
  else if (iparam ==2) {
    ...
    Kref = 0.13726;
    Mref = 0.0525;      // GeV^2
    KerrRef = 0.00081;  // matched to the expected 1sigma in the ideal case
    MerrRef = 0.0030;   //  "   "
    if (iLumi==1) {
      KerrRef = 0.045;  // matched to the expected 1sigma in the ideal case for the PilotRun lumi = 3/pb
      MerrRef = 0.;     // 1D param 
    }
  }
  else {
